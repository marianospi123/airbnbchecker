...
import { parseDate, getAlert } from "../utils/utils";
...
export default function Reservas() {
  ...
  useEffect(() => {
    const cargarReservas = async () => {
      try {
        const res = await fetch(`${process.env.REACT_APP_API_URL}/reservas`);
      ...
      try {
        ...
        if (permiso === "granted") {
          ...
          if (token) {
            ...
            await fetch(`${process.env.REACT_APP_API_URL}/fcm-token`, {
              method: "POST",
  ...
  const enviarNotificacion = async (reserva) => {
    ...
    try {
      ...
      await fetch(`${process.env.REACT_APP_API_URL}/fcm-notify`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const editarReserva = async (idx, campo, valor) => {
    ...
    if (reserva.id) {
      try {
        await fetch(`${process.env.REACT_APP_API_URL}/reservas/${reserva.id}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  const borrarReserva = async (idx) => {
    ...
    if (reserva.id) {
      try {
        await fetch(`${process.env.REACT_APP_API_URL}/reservas/${reserva.id}`, { method: "DELETE" });
      } catch (err) {
  ...
  const importarExcel = async (e) => {
    ...
    try {
      const res = await fetch(`${process.env.REACT_APP_API_URL}/importar-excel`, {
        method: "POST",
        body: formData,
      });
      ...
      if (data.ok) {
        ...
        const res2 = await fetch(`${process.env.REACT_APP_API_URL}/reservas`);
              ...
              <tbody>
                {reservasMes.map((r, i) => (
                  ...
                  >
                    ...
                    <td className="px-3 py-2 border-b font-bold text-red-600">{getAlert(r)}</td>
                    <td className="px-3 py-2 border-b text-center">
                      <button
                        onClick={() => borrarReserva(reservas.findIndex((res) => res.id === r.id))}
